...
class Settings
{
    ...
    private function _getSections($section_type = '', $parent_id = 0, $generate_href = true, $use_access_level = true, $lang_code = '')
    {
        ...
        if (!empty($lang_code)) {
            ...
            $values .= ', ?:settings_descriptions.value as description, ?:settings_descriptions.original_value as original_description, object_id, object_type';
        } else {
        ...
        if (!empty($section_id)) {
            ...
            if (!empty($sections)) {
                ...
                if (!empty($setting_ids)) {
                    ...
                    $variant_ids =  db_get_fields("SELECT object_id FROM ?:settings_variants WHERE object_id IN (?n)",$setting_ids);
        ...
        if (function_exists($addon_variant_func)) {
            ...
            if (($object_id !== null && $object_id !== 0) || $object_id == 'all') {
                ...
                $_variants = db_get_array(
                    "SELECT ?:settings_variants.*, ?:settings_descriptions.value, ?:settings_descriptions.original_value, ?:settings_descriptions.object_type "
    ...
    private function _get_list($fields, $section_id = '', $section_tab_id = '', $no_headers = false, $extra_condition = '', $is_global = true, $company_id = null, $lang_code = '')
    {
        ...
        if (!empty($lang_code)) {
            ...
            $fields[] = db_quote('?:settings_descriptions.original_value as original_description');
