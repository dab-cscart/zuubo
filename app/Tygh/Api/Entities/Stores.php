...
class Stores extends AEntity
{
    ...
    private function updateCompanySettings($params, $company_id)
    {
        $settings = array();
    ...
    private function checkRequiredParams($params, $mode = 'update')
    {
        $result = array(
            'valid_params' => true,
            'message' => '',
        ...
        if (empty($params['storefront']) && $mode == 'add') {
    ...
    public function index($id = 0, $params = array())
    {
        ...
        if (!empty($id)) {
            ...
            $key = strtolower(array_pop(explode('\\', get_class($this))));
            $data = array(
                $key => $data,
                'params' => $params,
            );
    ...
    public function delete($id)
    {
        ...
        if (fn_delete_company($id)) {
            $status = Response::STATUS_OK;
            $data['message'] = 'Ok';
        } elseif (!fn_notification_exists('type', 'W')) {
            $status = Response::STATUS_NOT_FOUND;
        }
