...
class Features extends AEntity
{
    private function checkRequiredParams($params, $mode = 'update')
    {
        ...
        if ($mode == 'create') {
            ...
            if (empty($params['feature_type'])) {
                fn_set_notification('E', __('error'), __('api_need_feature_type'));
                $result = false;
            }

            if (empty($params['description'])) {
                fn_set_notification('E', __('error'), __('api_need_feature_name'));
                $result = false;
            }

            if (fn_allowed_for('ULTIMATE')) {
                if ((empty($params['company_id'])) && Registry::get('runtime.company_id') == 0) {
                    fn_set_notification('E', __('error'), __('api_need_store'));
                    $result = false;
                }
            }
        }
    ...
    public function create($params)
    {
        ...
        if ($this->checkRequiredParams($params, 'create')) {
