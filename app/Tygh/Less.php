...
class Less extends \lessc
{
     ...
     */
    public function customCompile($less_output, $dirname, $data = array(), $prepend_prefix = '')
    {
        // Apply all Custom styles styles
        $less_output .= Styles::factory(fn_get_theme_path('[theme]'))->getLess($data);
    ...
    public static function parseUrls($content, $from_path, $to_path)
    {
        if (preg_match_all("/url\((?![\"]?data\:).*?\)/", $content, $m)) {
            ...
            foreach ($m[0] as $match) {
                if (strpos($match, '://') !== false) { // skip absolute URLs
                    continue;
                }
