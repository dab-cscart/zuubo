...
class Layout extends CompanySingleton
{
    ...
    public function setDefault($layout_id)
    {
        ...
        $theme_name = db_get_field('SELECT theme_name FROM ?:bm_layouts WHERE layout_id = ?i ?p LIMIT 1', $layout_id, $condition);
        db_query('UPDATE ?:bm_layouts SET is_default = 0 WHERE theme_name = ?s ?p', $theme_name, $condition);
        db_query('UPDATE ?:bm_layouts SET is_default = 1 WHERE layout_id = ?i ?p', $layout_id, $condition);
    ...
    public function copyById($from_layout_id, $to_layout_id)
    {
        ...
        // Copy style
        db_query("UPDATE ?:bm_layouts SET style_id = ?s WHERE layout_id = ?i", $from_layout['style_id'], $to_layout_id);
