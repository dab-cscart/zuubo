    ...
    public static function outputTranslateWrapper($content, \Smarty_Internal_Template $template)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            foreach ($matches[0] as $k => $m) {
                ...
                if (strpos($m, 'class="') !== false) {
                    $class_added = str_replace('class="', 'data-ca-translate="' . $matches[3][$k] . $matches[4][$k] . '" class="cm-load-translate ', $phrase_replaced);
                } else {
                    $class_added = str_replace($matches[1][$k], $matches[1][$k] . ' data-ca-translate="' . $matches[3][$k] . $matches[4][$k] . '" class="cm-load-translate"', $phrase_replaced);
                }
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            foreach ($matches[0] as $k => $m) {
                ...
                if (strpos($m, 'class="') !== false) {
                    $class_added = str_replace('class="', 'data-ca-translate="' . $matches[4][$k] . $matches[5][$k] . '" class="cm-load-translate ', $phrase_replaced);
                } else {
                    $class_added = str_replace('<' . $matches[2][$k], '<' . $matches[2][$k] . ' data-ca-translate="' . $matches[4][$k] . $matches[5][$k] . '" class="cm-load-translate"', $phrase_replaced);
                }
                $content = str_replace($matches[0][$k], $class_added, $content);
            }
        }
        ...
        $replacement = '<var class="translate-wrap"><i class="cm-icon-translate icon-translate"></i><var data-ca-translate="$2$3" class="cm-translate translate-item">$4</var></var>';
        while (preg_match($pattern, $content, $matches)) {
            $phrase_replaced = $matches[0];
