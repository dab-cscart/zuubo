...
class Helpdesk
{
    ...
    public static function checkStoreImportAvailability($license_number, $version, $edition = PRODUCT_EDITION)
    {
        ...
        $data = Http::get(Registry::get('config.updates_server'), $request, array(
            'timeout' => 10
        ));

        if (empty($data)) {
            $data = fn_get_contents(Registry::get('config.updates_server') . '/index.php?' . http_build_query($request));
        }
