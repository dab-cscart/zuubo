...
class Api
{
    ...
    private $called_version = '1.0';

    private $fake_entities = array(
        'version' => array(
            'index' => 'getVersion',
        ),
    );
    ...
    private function defineArea()
    {
        ...
        if ($user_data) {
            if ($user_data['user_type'] == 'A') {
                ...
                /**
                 * Currently no access for customers.
                 */
     ...
     */
    private function getResponse($resource)
    {
        $response = null;
    ...
    * @return Response Response
    */
    private function exec($entity, $entity_properties)
    {
        $response = null;
     ...
     * @return bool              True on success, false otherwise
     */
    private function checkAccess($entity, $method_name)
    {
        $can_access = false;

        if ($entity instanceof \Tygh\Api\AEntity && method_exists($entity, $method_name)) {
     ...
     */
    private function reflectionParamsToArray($reflection_params)
    {
        $params = array();
    ...
    private function getObjectByEntity($entity_properties)
    {
        ...
        $entity = class_exists($class_name) ? new $class_name($this->auth) : null;
