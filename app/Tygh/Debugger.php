...
class Debugger
{
    ...
    public static function init($reinit = false)
    {
        ...
        switch (true) {
            ...
            case (!$reinit && (!empty(self::$debugger_cookie) || isset($_REQUEST['debug']))):
            ...
            case (isset($_REQUEST['debug'])):
                if ($_SESSION['auth']['user_type'] == 'A' && $_SESSION['auth']['is_root'] == 'Y') {
                    ...
                    self::$debugger_cookie = substr(md5(SESSION::getId() . $user_admin['email'] . $user_admin['password']), 0, 8);

                    $active_in_registry = true;
                    self::$active_debug_mode = true;
                }

                if (AREA == 'C' && !empty($_REQUEST['debug'])) {
                    if (!empty(self::$actives[$_REQUEST['debug']]) && (time() - self::$actives[$_REQUEST['debug']]) < 0) {
                        $active_in_registry = true;
                        self::$debugger_cookie = $_REQUEST['debug'];
                        self::$active_debug_mode = true;
                    }
                }
