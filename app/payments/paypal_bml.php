...
function fn_paypal_set_bml_checkout($payment_id, $order_id = 0, $order_info = array(), $cart = array())
{
    $processor_data = fn_get_payment_method_data($payment_id);

    if (!empty($order_id)) {
        $return_url = fn_url("payment_notification.notify?payment=paypal_bml&order_id=$order_id", 'C', 'current');
        $cancel_url = fn_url("payment_notification.cancel?payment=paypal_bml&order_id=$order_id", 'C', 'current');
    } else {
