...
if (defined('PAYMENT_NOTIFICATION') && !empty($_REQUEST['order_id'])) {
    ...
    if ($mode == 'return') {
        if (fn_check_payment_script('paypal_advanced.php', $order_id)) {
            ...
            <body onload="javascript: parent.location = ' . "'$url'" . ';">' .
            __('text_paypal_processing_payment')
            . '</body>
            </html>';
            fn_flush();
        }
...
<div align="center">
    <div>
        <iframe src="$iframe_url" height="450" width="500" frameborder="0" scrolling="no" ></iframe>
    </div>
    <div>
        <span class="button-big button-wrap-left"><span class="button-big button-wrap-right"><a href="$cancel_url">$cancel_text</a></span></span>
    </div>
</div>
EOT;
        fn_flush();
