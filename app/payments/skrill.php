...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($mode == 'return') {
        ...
        if (!empty($order_id)) {
            ...
            if (fn_check_payment_script('skrill.php', $order_id)) {
        ...
        if (fn_check_payment_script('skrill.php', $_REQUEST['order_id'])) {
...
//		$pp_response['mb_currency'] = $mb_currency;
    ...
    $post_data = array(
        ...
        'return_url' => fn_url("payment_notification.return?payment=skrill&order_id=$order_id$suffix", AREA, 'current'),
        'return_url_text' => '',
        'cancel_url' => fn_url("payment_notification.cancel?payment=skrill&order_id=$order_id$suffix", AREA, 'current'),

        'status_url' => fn_url("payment_notification.status?payment=skrill&order_id=$order_id$suffix", AREA, 'current'),
    ...
    if (!$post_data['amount']) {
        if (!empty($suffix)) {
            ...
            $url = fn_url("payment_notification.unsupported_currency?payment=skrill&order_id=$order_id", AREA, 'current');
            echo <<<EOT
