...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if (strpos($result, 'FP_') === false) {
        ...
        $but_name_place = __('place_order');
        $but_name_continue = __('continue_shopping');

        $continue_url = fn_url('checkout.checkout');
        $lib_url = Registry::get('config.current_location') . '/js/lib/jquery/jquery.min.js';
        ...
        $res = Registry::get('view')->fetch(Registry::get('runtime.root_template'));

        $head_html = '';
        if (preg_match('#<head>(.*?)</head>#is', $res, $matches)) {
            $head_html = $matches[1];
        }
...
<body>
    ...
    <span id="wrap_place_order_tab1" class="button-submit-big button-wrap-left">
        <span class="button-submit-big button-wrap-right">
            <input type="button" class="button alt" name="{$but_name_place}" id="place_order" value="{$but_name_place}" onclick="FP.CartIntegration.placeOrder();"/>
        </span>
    </span>
    <span id="wrap_place_order_tab1" class="button-submit-big button-wrap-left">
        <span class="button-submit-big button-wrap-right">
            <input type="button" class="button alt" name="{$but_name_continue}" id="continue_shopping" value="{$but_name_continue}" onclick="self.location = '{$continue_url}';"/>
        </span>
    </span>
    </div>
    ...
    function FuturePayResponseHandler(response)
    {
      ...
      self.location = '{$return_url}' + params;
    }
