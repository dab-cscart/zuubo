        ...
        foreach ($bonus['value'] as $p_data) {
            ...
            if (!fn_allowed_for('ULTIMATE') || (fn_allowed_for('ULTIMATE') && Registry::get('runtime.company_id') && (fn_check_company_id('products', 'product_id', $p_data['product_id'], Registry::get('runtime.company_id')) || fn_ult_is_shared_product($p_data['product_id'], Registry::get('runtime.company_id')) == 'Y'))) {
                ...
                foreach ($cart['products'] as $product) {
                    if ($p_data['product_id'] == $product['product_id'] && !empty($product_data[$p_data['product_id']]['extra']['exclude_from_calculate'])) {
                        $can_add = false;
                        break;
                    }
                }

                if ($can_add && $ids = fn_add_product_to_cart($product_data, $cart, $auth)) {
 ...
 */
function fn_status_is_positive($status) 
{
    return isset($status['params']) && isset($status['status']) && $status['params']['inventory'] == 'D' && $status['status'] != 'O';
}
