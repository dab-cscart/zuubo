...
function fn_compress_files($archive_name, $file_list, $dirname = '')
{
    ...
    $end_file_extension = '.tgz';

    if (preg_match("/\..+/", $archive_name, $matches)) {
        $end_file_extension = $matches[0] . $end_file_extension;
    }

    $temp_file_name = fn_normalize_path($dirname . '/' . $archive_name . "._tmp");
    try {
        $phar = new PharData($temp_file_name);
        ...
        $phar->compress(Phar::GZ, $end_file_extension);
...
{
    $content_type = fn_get_mime_content_type($archive_name);

    try {
        // We cannot use PharData for ZIP archives. All extracted data looks broken after extract.
        if (strpos($content_type, 'zip') !== false && strpos($content_type, 'gzip') === false) {
            $zip = new ZipArchive;
                ...
                if (is_dir($path)) {
                    ...
                    if (!empty($_files)) {
                        foreach ($_files as $_file) {
                            fn_fpt_chmod_file($path . '/' . $_file, $perm, false);
                        }
                    }
