    ...
    if ($mode == 'backup') {
        ...
        if (!fn_mkdir(Registry::get('config.dir.database'))) {
            fn_set_notification('E', __('error'), __('text_cannot_create_directory', array(
                '[directory]' => Registry::get('config.dir.database')
            )));
            exit;
        }
        ...
        if ($_REQUEST['dbdump_compress'] == 'Y') {
            ...
            $result = fn_compress_files($dbdump_filename, $dbdump_filename, dirname($dump_file));
            unlink($dump_file);
        }
...
if ($mode == 'getfile' && !empty($_REQUEST['file'])) {
    ...
    Registry::get('view')->assign('backup_dir', Registry::get('config.dir.database'));
...
function fn_restore_dump($files)
{
    ...
    foreach ($files as $file) {
        ...
        if (strpos($file, '.tgz') !== false) {
            fn_decompress_files(Registry::get('config.dir.database') . $file, Registry::get('config.dir.database'));
            $file = substr($file, 0, strpos($file, '.tgz'));
        }

        db_import_sql_file(Registry::get('config.dir.database') . $file);
    }
