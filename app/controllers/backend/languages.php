...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'install_from_po') {
        $uploaded_data = fn_filter_uploaded_data('language_data');
...
if ($mode == 'manage') {
    ...
    $edition_pack_path = Registry::get('config.dir.lang_packs') . 'editions/' . fn_basename($_REQUEST['pack'], '.po') . '_' . fn_get_edition_acronym(PRODUCT_EDITION) . '.po';

    if (Languages::installLanguagePack($pack_path)) {
        // If we have language variables for edition, install it
        if (file_exists($edition_pack_path) && is_readable($edition_pack_path)) {
            Languages::installLanguagePack($edition_pack_path, true);
        }
