...
fn_define('DIR_REPOSITORY', 'var/themes_repository/basic/');
...
$base_ult_theme_files = array(
    'customer/exception.tpl',
);
...
if ($mode == 'manage') {
    ...
    if (!fn_allowed_for('ULTIMATE:FULL:TRIAL')) {
        ...
        if (empty($uc_settings['license_number']) && !fn_allowed_for('ULTIMATE:FREE')) {
            ...
            $check_file_hash = fn_get_contents(Registry::get('config.updates_server') . '/index.php?dispatch=product_updates.get_fh_code');
 ...
 */
    ...
    if (empty($xml)) {
        ...
        fn_uc_run_continuous_job('fn_uc_create_themes', array(0, Registry::get('config.dir.upgrade') . $_SESSION['uc_package'] . '/package', $_SESSION['uc_package'], $skip_files, $custom_theme_files, $base_ult_theme_files));

        if (fn_allowed_for('ULTIMATE')) {
            $company_ids = fn_get_all_companies_ids();
            foreach ($company_ids as $company_id) {
                fn_uc_run_continuous_job('fn_uc_create_themes', array($company_id, Registry::get('config.dir.upgrade') . $_SESSION['uc_package'] . '/package', $_SESSION['uc_package'], $skip_files, $custom_theme_files, $base_ult_theme_files));
            }
        }

        fn_uc_check_files(Registry::get('config.dir.upgrade') . $_SESSION['uc_package'] . '/package', $hash_table, $result, $_SESSION['uc_package'], $custom_theme_files);
    ...
    if (!empty($new_license)) {
        ...
        if ($is_license_expired) {
            fn_set_notification('E', __('error'), __('update_period_expired', array(
                '[url]' => Registry::get('config.updates_server')
            )));
        }
...
function fn_uc_restore_settings($new_edition = '')
{
    ...
    if (!empty($was_settings_backed_up)) {
        ...
        foreach ($settings as $setting) {
            Settings::instance()->updateValue($setting['name'], $setting['value'], $setting['section_name']);
        }
        ...
        if (!empty($was_company_settings_backed_up)) {
            ...
            foreach ($company_settings as $setting) {
                Settings::instance()->updateValue($setting['name'], $setting['value'], $setting['section_name'], false, $setting['company_id']);
            }
...
function fn_uc_get_package($package_id, $md5, $package, $uc_settings, $backend_files)
{
    ...
    $data = fn_get_contents(Registry::get('config.updates_server') . '/index.php?dispatch=product_updates.get_package&package_id=' . $package_id . '&edition=' . PRODUCT_EDITION . '&license_number=' . $uc_settings['license_number']);
    if (!empty($data)) {
        ...
        if (file_exists($original_file)) {
            if (md5_file($original_file) != md5_file($path)) {
                ...
                if (strpos($relative_file, 'design/themes/') === 0) {
                    ...
                    // replace all themes except basic
                    if (fn_uc_check_array_value($relative_file, $custom_theme_files) && strpos($relative_file, '/basic/') === false) {
 ...
 * @param array $custom_theme_files list of custom theme files
 * @param array $base_ult_theme_files list of files in the themes dir for the base store (ultimate edition only)
 * @return boolean true if structured created correctly, false - otherwise
 */
function fn_uc_create_themes($company_id, $path, $package, $skip_files, $custom_theme_files, $base_ult_theme_files, $log)
{
    static $theme_names_customer = array();
    static $themes_paths = array();
    static $installed_themes = array();

    if (empty($theme_names_customer[$company_id])) {
        $theme_names_customer[$company_id] = fn_get_theme_path('[theme]', 'C', $company_id);
    }

    if (empty($themes_paths[$company_id])) {
        $themes_paths[$company_id] = fn_get_theme_path('[relative]', 'C', $company_id);
    }

    if (empty($installed_themes[$company_id])) {
        $installed_themes[$company_id] = fn_get_dir_contents(Registry::get('config.dir.root') . '/' . $themes_paths[$company_id], true, false);
    }
    ...
    if (is_file($path)) {
        ...
        if (strpos($path, DIR_REPOSITORY) !== false) {
            ...
            // frontend theme
            if (fn_allowed_for('MULTIVENDOR') || fn_allowed_for('ULTIMATE') && ($company_id || !$company_id && (strpos($path, DIR_REPOSITORY . 'mail/') !== false))) {
                foreach ($installed_themes[$company_id] as $s) {
                    $log->write('Processing theme: ' . $s, __FILE__, __LINE__);
                    if (!fn_uc_check_array_value($path, $custom_theme_files) || $s = 'basic') { // copy non-custom files only
                        $files[] = str_replace(Registry::get('config.dir.upgrade') . $package . '/package/' . DIR_REPOSITORY, Registry::get('config.dir.upgrade') . $package . '/package/' . $themes_paths[$company_id] . '/' . $s . '/', $path);
                    }
                }
            }
        // Copy data from alternative themes
        } elseif (strpos($path, 'var/themes_repository/' . $theme_names_customer[$company_id]) !== false) {
            $files[] = str_replace(Registry::get('config.dir.upgrade') . $package . '/package/var/themes_repository/', Registry::get('config.dir.upgrade') . $package . '/package/' . $themes_paths[$company_id] . '/', $path);
        }
    ...
    if (is_dir($path)) {
        ...
        while (false !== ($entry = $dir->read())) {
            ...
            fn_uc_create_themes($company_id, rtrim($path, '/') . '/' . $entry, $package, $skip_files, $custom_theme_files, $base_ult_theme_files, $log);
        }
        // Clean up
        $dir->close();
...
/**
 * Function finds files from basic theme in all installed themes and return full path to those files.
 *
