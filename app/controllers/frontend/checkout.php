...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'add') {
        ...
        if (md5(serialize($cart['products'])) != $previous_state && empty($cart['skip_notification'])) {
            ...
            if (!empty($added_products)) {
                ...
                if (Registry::get('config.tweaks.disable_dhtml') && Registry::get('settings.General.redirect_to_cart') == 'Y') {
        ...
        if (Registry::get('config.tweaks.disable_dhtml') && Registry::get('settings.General.redirect_to_cart') == 'Y' && !defined('AJAX_REQUEST')) {
    ...
    if ($mode == 'apply_coupon') {
        ...
        $cart['pending_coupon'] = trim($_REQUEST['coupon_code']);
        $cart['recalculate'] = true;
    ...
    if ($mode == 'add_profile') {
        ...
        if (list($user_id, $profile_id) = fn_update_user(0, $_REQUEST['user_data'], $auth, false, true)) {
            ...
            db_query("DELETE FROM ?:user_session_products WHERE session_id = ?i AND type = ?s AND user_type = ?s", Session::getId(), 'C', 'U');
