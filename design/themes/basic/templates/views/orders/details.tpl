    ...
    <div id="content_shipment_info" class="orders-shipment {if $selected_section != "shipment_info"}hidden{/if}">
        {foreach from=$shipments key="id" item="shipment"}
            ...
            {if $shipment.carrier && $shipment.tracking_number}
                ...
                {$shipment.shipping}&nbsp;({__("tracking_num")}{if $smarty.capture.carrier_url|trim != ""}<a {if $smarty.capture.carrier_url|strpos:"://"}target="_blank"{/if} href="{$smarty.capture.carrier_url nofilter}">{/if}{$shipment.tracking_number}{if $smarty.capture.carrier_url|trim != ""}</a>{/if})
            {else}
                {$shipment.shipping}
            {/if}
            </div>
