...
<tr>
    ...
    <th style="width: 15%"><a class="{$ajax_class}" href="{"`$c_url`&sort_by=total&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("savings")}</a>{if $search.sort_by == "total"}{$sort_sign nofilter}{/if}</th>
</tr>
</thead>
...
<tr {cycle values=",class=\"table-row\""}>
    ...
    <td>{include file="common/price.tpl" value=$o.savings}</td>
</tr>
{foreachelse}
<tr>
...
<table>
    <tr>
        ...
        <td data-ct-orders-summary="summary-subtotal">{include file="common/price.tpl" value=$totals.gross_total}</td>
    </tr>
    <tr>
        <td>{__("totally_paid")}:&nbsp;</td>
        <td data-ct-orders-summary="summary-subtotal">{include file="common/price.tpl" value=$totals.totally_paid}</td>
    </tr>
    <tr class="total">
        <td>{__("total_savings")}:&nbsp;</td>
        <td data-ct-orders-summary="summary-total">{include file="common/price.tpl" value=$totals.savings}</td>
    </tr>
</table>
    </div>

{capture name="mainbox_title"}{__("savings")}{/capture}