    ...
    <td style="width: 33%">
        ...
        {elseif $filter.feature_type == "O" || $filter.feature_type == "N" || $filter.feature_type == "D" || $filter.condition_type == "D" || $filter.condition_type == "F"}
            ...
            {/if}
                ...
                {/if}
                <p{if !$filter.slider} class="select-field"{/if}>
		{if $filter.field_type != 'C'}
		    <input type="radio" name="variants[{$el_id}]" id="no_ranges_{$el_id}" value="" checked="checked" class="radio" /><label for="no_ranges_{$el_id}">{__("none")}</label>
		{/if}
		</p>
                {if !$filter.slider}
                    {foreach from=$filter.ranges item="range"}
			    ...
			    {if $filter.field_type == 'C'}
				<input type="checkbox" class="checkbox" name="variants[]" id="variants_{$range.range_id}" value="[C{$range.range_id}]" {if "[C`$range.range_id`]"|in_array:$search.variants}checked="checked"{/if} /><label for="ranges_{$el_id}{$range.range_id}">{$range.range_name|fn_text_placeholders}</label>
			    {else}
				<input type="radio" class="radio" name="variants[{$el_id}]" id="ranges_{$el_id}{$range.range_id}" value="{$_type}{$range.range_id}" {if $search.variants.$el_id == "`$_type``$range.range_id`"}checked="checked"{/if} /><label for="ranges_{$el_id}{$range.range_id}">{$range.range_name|fn_text_placeholders}</label>
			    {/if}
			</div>
                    {/foreach}
                {/if}
