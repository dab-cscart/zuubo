        ...
        <tr>
            ...
            <td>
                {if $ul.action == $smarty.const.CHANGE_DUE_ORDER}
                    ...
                    {__("order")}&nbsp;{if $order_exist}<a href="{"orders.details?order_id=`$reason.order_id`"|fn_url}" class="underlined">{/if}<span>#{$reason.order_id}</span>{if $order_exist}</a>{/if}{if $product_exist},&nbsp;{__("product")}&nbsp;<a href="{"products.update?product_id=`$reason.product_id`"|fn_url}" class="underlined"><span>{$product_exist}</span></a>{/if}:&nbsp;{$statuses[$reason.from]}&nbsp;&#8212;&#8250;&nbsp;{$statuses[$reason.to]}{if $reason.text}&nbsp;({__($reason.text)}){/if}
                ...
                {elseif $ul.action == $smarty.const.CHANGE_DUE_ORDER_DELETE}
                    {assign var="reason" value=$ul.reason|unserialize}
                    {__("order")} <span>#{$reason.order_id}</span>{if $product_exist},&nbsp;{__("product")}&nbsp;<a href="{"products.update?product_id=`$reason.product_id`"|fn_url}" class="underlined"><span>{$product_exist}</span></a>{/if}: {__("deleted")}
                {elseif $ul.action == $smarty.const.CHANGE_DUE_ORDER_PLACE}
                    {assign var="reason" value=$ul.reason|unserialize}
                    {assign var="order_exist" value=$reason.order_id|fn_get_order_name}
                    {__("order")} {if $order_exist}<a href="{"orders.details?order_id=`$reason.order_id`"|fn_url}" class="underlined">{/if}<span>#{$reason.order_id}</span>{if $order_exist}</a>{/if}{if $product_exist},&nbsp;{__("product")}&nbsp;<a href="{"products.update?product_id=`$reason.product_id`"|fn_url}" class="underlined"><span>{$product_exist}</span></a>{/if}: {__("placed")}
                {else}
