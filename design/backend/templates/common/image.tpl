...
{if $image_data.image_path}
    <img {if $image_id}id="image_{$image_id}"{/if} src="{$image_data.image_path}" width="{$image_data.width}" height="{$image_data.height}" alt="{$image_data.alt}" {if $image_data.generate_image}class="spinner cm-generate-image" data-ca-image-path="{$image_data.image_path}"{/if} title="{$image_data.alt}" />
{else}
