...
<ul>
    {foreach from=$items item="item" name="it" key="item_id"}
        ...
        <li class="{if $item.name == $active_object}parent{/if} {if $last_object && $item.name == $active_object}active{/if}">
            <a data-ca-item-full-path="{$current_path|rtrim:'/'}/{$item.name}" data-ca-item-path="{$current_path|rtrim:'/'}" data-ca-item-filename="{$item.name}" {if $item.ext}data-ca-item-ext="{$item.ext}"{/if} data-ca-item-type="{$item.type}" data-ca-item-perms="{$item.perms}" class="cm-te-file" id="file_id_{$uniqid}">
