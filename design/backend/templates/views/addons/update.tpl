    ...
    <div class="cm-tabs-content" id="tabs_content_{$_addon}">
        ...
        <div id="content_{$_addon}_{$section}" class="settings{if $subsections.$section.type == "SEPARATE_TAB"} cm-hide-save-button{/if}">
            ...
            {foreach from=$field_item key="name" item="data" name="fe_addons"}
                {include file="common/settings_fields.tpl" item=$data section=$_addon html_id="addon_option_`$_addon`_`$data.name`" html_name="addon_data[options][`$data.object_id`]" index=$smarty.foreach.fe_addons.iteration total=$smarty.foreach.fe_addons.total class="setting-wide"}
            {/foreach}
        </div>
