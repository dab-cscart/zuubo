(function(_, $) {
    var methods = {
	    init: function(params) {
	        ...
	        default_params['ranges'][_.tr('last_7_days')] = [moment().startOf('day').subtract('days', 6), moment().endOf('day')];
	        default_params['ranges'][_.tr('last_30_days')] = [moment().startOf('day').subtract('days', 29), moment().endOf('day')];
	            ...
	            elm.daterangepicker(params,
	            	function(start, end) {
					    ...
					    if (self.data('ca-target-url') && self.data('ca-target-id')) {
					    	$.ceAjax('request', $.attachToUrl(self.data('ca-target-url'), 'time_from=' + (start.valueOf() / 1000) + '&time_to=' + (parseInt(end.valueOf() / 1000) + 86400)), { // Seconds in day
